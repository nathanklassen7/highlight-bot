...
def collect_session_or_all(interaction: BlockInteraction, collect_all: Optional[bool] = False):
    ...
    def _upload_and_finalize():
        try:
            upload_videos_sequentially(upload_files_to_thread, files_to_send)
            ...
            interaction.update_with_message(f"Error uploading clips: {e}")
        finally:
            file_read_lock.release()

    threading.Thread(target=_upload_and_finalize).start()
