...
def encode_video(audio_start_time, video_start_time):
    ...
    return call([f'ffmpeg -r {fps} -i {VIDEO_BUFFER_FILE} -i {AUDIO_BUFFER_FILE} -c:v copy -af "adelay={offset}|{offset},pan=mono|c0=FL" -c:a aac -ac 1 {output_file} -y -shortest'], shell=True,stdout=DEVNULL, stderr=DEVNULL)
